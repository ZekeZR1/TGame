...
bool AIMSelect::Start()
{
	...
	m_ok->Init(L"Assets/sprite/GUI/button/sb_OK.dds", 151, 64,true);
	m_odum = NewGO<SpriteRender>(9, "sp");
	m_odum->Init(L"Assets/sprite/GUI/button/simple_button.dds", 151, 64);
	CVector3 pos = { 320,-210,0 };
	m_ok->SetPosition(pos);
	m_odum->SetPosition(pos);

	m_can = NewGO<SpriteRender>(10, "sp");
	m_can->Init(L"Assets/sprite/GUI/button/sb_cancel.dds", 151, 64,true);
	m_cdum = NewGO<SpriteRender>(9, "sp");
	m_cdum->Init(L"Assets/sprite/GUI/button/simple_button.dds", 151, 64);
	pos = { 420,-210,0 };
...
void AIMSelect::Update()
{
	...
	{
		if(m_issel)
			m_cdum->Init(L"Assets/sprite/GUI/button/simple_button_blue.dds", 151, 64);
	...
	{
		if(m_issel)
			m_odum->Init(L"Assets/sprite/GUI/button/simple_button_blue.dds", 151, 64);
