...
bool AIEditNodeBackGround::Start()
{
	...
	CVector2 m_pos2 = CVector2::Zero();          //フォントの座標。
	m_pos2.x -= 560.f;
	m_pos2.y += 360.f;

	m_fonts[0]->Init(L"簡単AI制作モード", { m_pos2 }, 0.0, CVector4::White, 0.78, { 0.0,0.0 });
	...
	m_fonts[7]->DrawShadow({ SetShadowPos });

	m_pos2.y -= 85.f;
	m_fonts[8]->Init(L"8", { m_pos2 }, 0.0, CVector4::White, 0.9, { 0.0,0.0 });
	m_fonts[8]->DrawShadow({ SetShadowPos });
